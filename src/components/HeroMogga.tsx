  ...
  return (
    <section className={styles.hero} aria-label="Mogga hero">
      ...
      <img
        ...
        src="/ui/hero/mammoth-hero.png"
        alt="Mammoth on a stone throne"
        draggable={false}
      />

      {/* Плавающая панель — позиция задаётся переменными в CSS */}
      <div className={styles.panel}>
        ...
        {/* было <button ...> — теперь пассивный блок */}
        <div className={`${styles.btn} ${styles.btnCA} ${styles.btnStatic}`} aria-hidden="true">
            <span className={styles.btnLabel}>CA: {CA}</span>
        </div>
          <button type="button" className={`${styles.btn} ${styles.btnCopy}`} onClick={onCopy} aria-live="polite">
